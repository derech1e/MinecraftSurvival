...
public class UserChannelListener extends ListenerAdapter {
    ...
    public void onGuildVoiceLeave(@NotNull GuildVoiceLeaveEvent event) {
        ...
        BotMain.logger.info(String.format("'%s' << '%s'. Aktuell: %s", liftedUser, event.getChannelLeft().getName(), currentUserInChannel));
    }
    ...
    public void onGuildVoiceJoin(@NotNull GuildVoiceJoinEvent event) {
        ...
        BotMain.logger.info(String.format("'%s' >> '%s'. Aktuell: %s", joinedUser, event.getChannelJoined().getName(), currentUserInChannel));
    }
    ...
    public void onGuildVoiceMove(@NotNull GuildVoiceMoveEvent event) {
        ...
        List<String> currentUserInChannel = event.getChannelJoined().getMembers().stream().map(Member::getUser).map(User::getName).collect(Collectors.toList());
        BotMain.logger.info(String.format("'%s' bewegt von '%s' in '%s'. Aktuell: %s", user, event.getChannelLeft().getName(), event.getChannelJoined().getName(), currentUserInChannel));
    }
}
